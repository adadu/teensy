...
/* JEA slight alteration
   select MIDIx16 to enable enough ports for all serial and 8 ch on on usb hubs
 */
...
/* MIDI_CREATE_CUSTOM_INSTANCE(HardwareSerial, Serial7, MIDI7, MySettings); */
 ...
 /*     MIDI1.sendSysEx(SysExLength, MIDI4.getSysExArray(), true);
  ...
  for (int port=0; port < 8; port++) {
    if (midilist[port]->read()) {
      ...
      MIDI5.send(type, data1, data2, channel);    
      MIDI6.send(type, data1, data2, channel);
      midi02.send(type, data1, data2, channel);
